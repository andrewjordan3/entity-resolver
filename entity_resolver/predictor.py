...
class EntityPredictor:
    ...
    def predict(
        ...
        if cluster_model is None or canonical_map is None:
            raise ValueError(
                'A fitted cluster_model and canonical_map must be provided for prediction.'
            )
    ...
    def _add_cluster_probabilities(
        ...
        gdf = gdf.merge(prob_map, on='final_cluster', how='left').rename(
            columns={'avg_cluster_prob': 'cluster_probability'}
        )
