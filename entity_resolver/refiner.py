...
from .config import OutputConfig, ValidationConfig, VectorizerConfig
from .utils import (
...
class ClusterRefiner:
    ...
    def build_canonical_map(self, gdf: cudf.DataFrame) -> cudf.DataFrame:
        ...
        logger.debug(
            'Step 1: Determining best address profile for each cluster via groupby-apply...'
        )
        ...
        avg_probabilities_df = (
            clustered_records_gdf.groupby('final_cluster')
            .agg(avg_cluster_prob=('cluster_probability', 'mean'))
            .reset_index()
        )
        ...
        ).merge(canonical_names_df, on='final_cluster', how='left')
    ...
    def apply_canonical_map(
        self, gdf: cudf.DataFrame, canonical_map: cudf.DataFrame
    ) -> cudf.DataFrame:
        """
        ...
        """
        logger.info(f'Applying canonical map to {len(gdf):,} records in final dataset...')

        # Handle empty canonical map case
        if canonical_map is None or canonical_map.empty:
            logger.warning('Canonical map is empty. Assigning self-references for all records.')
        ...
        result_dataframe = gdf.merge(canonical_merge_columns, on='final_cluster', how='left')
    ...
    def _enrich_addresses(self, gdf: cudf.DataFrame) -> cudf.DataFrame:
        ...
        canonical_profiles_df = clustered_gdf.groupby('cluster').apply(get_best_address_gpu)
        ...
        cols_to_merge = [
            col for col in canonical_merge_cols.keys() if col in canonical_profiles_df.columns
        ]
        ...
        canonical_info_exists = (gdf['canonical_street_name'].notna()) & (
            gdf['canonical_street_name'] != ''
        )
        geo_match_mask = (
        ...
        eligible_mask = (
            (gdf['cluster'] != -1) & missing_street_mask & canonical_info_exists & geo_match_mask
        )
        ...
        if num_enrichable > 0:
            ...
            gdf.loc[eligible_mask, 'addr_street_number'] = gdf.loc[
                eligible_mask, 'canonical_street_number'
            ]
            gdf.loc[eligible_mask, 'addr_street_name'] = gdf.loc[
                eligible_mask, 'canonical_street_name'
            ]
    ...
    def _split_clusters_by_conflict(self, gdf: cudf.DataFrame) -> cudf.DataFrame:
        ...
        if self.validation_config.enforce_state_boundaries:
            ...
            state_diversity_by_cluster = (
                valid_clusters_df.groupby('final_cluster')['addr_state']
                .nunique()
            ...
            if len(multi_state_clusters) > 0:
                ...
                new_cluster_assignments = multi_state_records_df.groupby(
                    ['final_cluster', 'addr_state']
                ).ngroup()
        ...
        if len(valid_clusters_with_address) > 0:
            ...
            if len(conflicting_address_ranges) > 0:
                ...
                conflict_records_df = gdf.loc[gdf['has_range_conflict']]

                if len(conflict_records_df) > 0:
                    # Create new cluster IDs for each unique address
                    new_range_based_clusters = conflict_records_df.groupby(
                        ['final_cluster', 'addr_normalized_key']
                    ).ngroup()

                    # Apply new cluster IDs
                    gdf.loc[gdf['has_range_conflict'], 'final_cluster'] = (
                        current_max_cluster_id + 1 + new_range_based_clusters
                    )
    ...
    def _number_chain_entities(self, canonical_map_gdf: cudf.DataFrame) -> cudf.DataFrame:
        ...
        address_counts_per_name = (
            canonical_map_gdf.groupby('canonical_name')
        ...
        chain_entity_names = address_counts_per_name.loc[
            address_counts_per_name['unique_address_count'] > 1, 'canonical_name'
        ]
        ...
        total_chain_locations = address_counts_per_name.loc[
            address_counts_per_name['unique_address_count'] > 1, 'unique_address_count'
        ].sum()
        ...
        unique_chain_locations = (
            canonical_map_gdf.loc[chain_records_mask, ['canonical_name', 'addr_normalized_key']]
            .drop_duplicates()
        ...
        unique_chain_locations['location_number'] = (
            unique_chain_locations.groupby('canonical_name').cumcount().astype('int32') + 1
        )

        # Merge location numbers back to canonical map
        enhanced_canonical_map = canonical_map_gdf.merge(
            unique_chain_locations, on=['canonical_name', 'addr_normalized_key'], how='left'
        )
        ...
        if has_location_number_mask.any():
            ...
            location_suffix = ' - ' + enhanced_canonical_map.loc[
                has_location_number_mask, 'location_number'
            ...
            logger.info(f'Numbered {has_location_number_mask.sum():,} chain entity locations.')
