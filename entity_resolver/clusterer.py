...
class EntityClusterer:
    ...
    def __init__(self, config: ClustererConfig) -> None:
        ...
        """
        self.config = config
        self.cluster_model: HDBSCAN | None = None
    ...
    def fit_transform(
        self, gdf: cudf.DataFrame, vectors: cp.ndarray
        ...
        """
        logger.info(
            f'Starting fit_transform with {len(gdf):,} records, {vectors.shape[1]} features'
        )
    ...
    def transform(
        ...
        if not self.umap_ensemble or not self.cluster_model:
            raise RuntimeError('Models not fitted. Call fit_transform() before transform().')
    ...
    def _predict_clusters(self, gdf: cudf.DataFrame, reduced_vectors: cp.ndarray) -> cudf.DataFrame:
        """
        ...
        if not self.cluster_model:
            raise RuntimeError('HDBSCAN model not fitted. Call fit_transform() first.')
        ...
        predicted_clusters = cudf.Series(
            self.cluster_model.predict(reduced_vectors), index=gdf.index
        )
        gdf['cluster'] = predicted_clusters.fillna(-1).astype('int32')

        # Assign default confidence for predictions
        default_conf = float(self.config.ensemble_params.default_rescue_conf)
        gdf['cluster_probability'] = cp.where(gdf['cluster'] != -1, default_conf, 0.0)
    ...
    def _run_hdbscan(self, gdf: cudf.DataFrame, reduced_vectors: cp.ndarray) -> cudf.DataFrame:
        """
        ...
        logger.info(
            f'HDBSCAN results: {n_clusters} clusters, {n_noise:,} noise points ({noise_rate:.1%})'
        )
    ...
    def _run_snn_engine(
        self, reduced_vectors: cp.ndarray, hdbscan_labels: cp.ndarray = None
    ) -> cp.ndarray:
        """
        ...
        initial_labels[partitions_df['vertex'].values] = (
            partitions_df['partition'].astype(cp.int32).values
        )
        ...
        labels_after_attachment = attach_noise_points(
            vectors_norm, labels_to_use, **self.config.noise_attachment_params.model_dump()
        )
        ...
        final_labels = merge_snn_clusters(vectors_norm, labels_after_attachment, merge_params)

        # Log final SNN statistics
        n_final_clusters = (
            int(final_labels[final_labels != -1].max() + 1) if (final_labels != -1).any() else 0
        )
    ...
    @gpu_memory_cleanup
    def _run_umap_ensemble(self, vectors: cp.ndarray, is_training: bool) -> cp.ndarray:
        """
            ...
            for run_idx in range(n_runs):
                ...
                try:
                    ...
                    expanded_embedding = self._expand_umap_embeddings(
                        unique_embedding, inverse_indices
                    )
    ...
    def _ensemble_cluster_labels(
        self, gdf_hdbscan: cudf.DataFrame, snn_labels: cp.ndarray
    ) -> cudf.DataFrame:
        """
        ...
        """
        assert len(gdf_hdbscan) == len(snn_labels), (
            f'Length mismatch: {len(gdf_hdbscan)} vs {len(snn_labels)}'
        )
        ...
        rescued_count = self._rescue_noise_points(
            final_labels, final_probs, noise_mask, snn_labels, snn_to_hdb_map, params
        )
    ...
    def _find_snn_to_hdbscan_mapping(
        self, hdb_labels: cp.ndarray, snn_labels: cp.ndarray, params: EnsembleParams
    ) -> cudf.DataFrame:
        """
        ...
        best_matches = overlap_counts.sort_values(
            ['snn', 'overlap'], ascending=[True, False]
    ...
    def _rescue_noise_points(
        ...
        if num_rescued > 0:
            ...
            final_probs[rescue_mask] = cp.asarray(default_rescue_confidence, dtype=cp.float32)
    ...
    def _mint_new_clusters(
        ...
        next_available_cluster_id = (
            0 if max_existing_cluster_id < 0 else max_existing_cluster_id + 1
        )
        ...
        max_snn_in_candidates = (
            int(cp.max(snn_cluster_ids_to_mint)) if len(snn_cluster_ids_to_mint) > 0 else -1
        )
        ...
        logger.debug(
            f'Minted {num_clusters_actually_minted:,} new clusters from unmapped SNN groups.'
        )
    ...
    def _generate_run_parameters(self, run_index: int, rng: np.random.Generator) -> dict[str, Any]:
        """
        ...
        if is_local_view:
            ...
            min_dist_range = self._get_sub_range(
                sampling_config.min_dist, start_percent=0.0, end_percent=0.6, use_log_scale=True
            )

            # Use lower 60% of spread range (linear scale for additive effect)
            spread_range = self._get_sub_range(
                sampling_config.spread, start_percent=0.0, end_percent=0.6, use_log_scale=False
            )
            ...
            min_dist_range = self._get_sub_range(
                sampling_config.min_dist, start_percent=0.4, end_percent=1.0, use_log_scale=True
            )

            # Use upper 60% of spread range
            spread_range = self._get_sub_range(
                sampling_config.spread, start_percent=0.4, end_percent=1.0, use_log_scale=False
            )
    ...
    def _validate_inputs(self, gdf: cudf.DataFrame, vectors: cp.ndarray) -> None:
        ...
        if len(gdf) != vectors.shape[0]:
            raise ValueError(
                f"DataFrame length ({len(gdf)}) doesn't match vector rows ({vectors.shape[0]})"
            )
    ...
    def _deduplicate_vectors(
        self, vectors: cp.ndarray, epsilon: float = 1e-9
        ...
        logger.debug(
            f'Deduplicating {n_samples:,} vectors with {n_features} features, epsilon={epsilon}'
        )
        ...
        merged_df = vectors_df[column_names].merge(
            unique_df_for_merge[column_names + ['unique_idx']], on=column_names, how='left'
        )
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            n_unique_rounded = int(
                len(vectors_df.drop_duplicates(subset=column_names, keep='first'))
            )
            ...
            try:
                if n_samples * n_features <= 5_000_000:  # ~ heuristic to avoid huge frames
                    ...
                    n_unique_exact = int(
                        len(df_exact.drop_duplicates(subset=column_names, keep='first'))
                    )
                else:
                    logger.debug(
    ...
    def _expand_umap_embeddings(
        self, unique_embeddings: cp.ndarray, inverse_indices: cp.ndarray
    ) -> cp.ndarray:
        """
    ...
    def _log_clustering_stats(self, gdf: cudf.DataFrame) -> None:
        ...
        logger.info(f'{"=" * 60}')
        logger.info('Final clustering statistics:')
    ...
    def get_models(self) -> dict[str, Any]:
        ...
        return {'cluster_model': self.cluster_model, 'umap_reducer_ensemble': self.umap_ensemble}

    def set_models(self, cluster_model: HDBSCAN, umap_reducer_ensemble: list[UMAP]) -> None:
        """
        ...
        logger.info(
            f'Loaded pre-trained models: HDBSCAN and {len(umap_reducer_ensemble)} UMAP models'
        )
