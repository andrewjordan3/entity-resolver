...
class TextNormalizer:
    ...
    def __init__(
        ...
        self.suffixes_to_remove = (
            set(config.suffixes_to_remove) if config.suffixes_to_remove else set()
        )
    ...
    def _dedup_config_overlap(self):
        ...
        if overlaps:
            logger.debug(
                f'Found {len(overlaps)} overlapping entries between replacements and suffixes'
            )
    ...
    def normalize_text(self, gdf: cudf.DataFrame, entity_col: str) -> cudf.DataFrame:
        ...
        for pattern_key in ['triple', 'double']:
            if pattern_key in self._compiled_patterns:
                ...
                normalized = normalized.str.replace_with_backrefs(
                    pattern_info['pattern'], pattern_info['replacement']
                )
        ...
        for key in ['ampersand', 'plus', 'forward_slash', 'pipe', 'middle_dot', 'bullet', 'dashes']:
            if key in self._compiled_patterns:
                ...
                normalized = normalized.str.replace(
                    pattern_info['pattern'], pattern_info['replacement'], regex=True
                )
        ...
        if too_short_mask.any():
            ...
            logger.warning(
                f'  {count_too_short:,} strings became too short, using minimal normalization'
            )
        ...
        if logger.isEnabledFor(logging.DEBUG) and len(gdf) > 0:
            ...
            for _, row in enumerate(sample_df.to_pandas().itertuples()):
    ...
    def consolidate_by_address(self, gdf: cudf.DataFrame) -> cudf.DataFrame:
        ...
        if num_empty_addresses > 0:
            logger.warning(
                f'Found {num_empty_addresses:,} records with empty addresses to be skipped.'
            )
        ...
        names_per_address = valid_records_gdf.groupby('addr_normalized_key')[
            'normalized_text'
        ].nunique()
        ...
        logger.info(
            f'Found {num_to_consolidate:,} addresses with multiple name variations to consolidate.'
        )
        ...
        logger.info(
            f'Processing {len(consolidation_subset_gdf):,} records across {num_to_consolidate:,} addresses.'
        )
        ...
        self._consolidation_stats['unique_after'] = int(
            consolidated_gdf['normalized_text'].nunique()
        )
        self._log_consolidation_summary()
    ...
    def _build_canonical_mapping(self, consolidation_subset_gdf: cudf.DataFrame) -> cudf.DataFrame:
        ...
        logger.debug(
            'Grouping names by address and applying canonical selection function on GPU...'
        )
        grouped_names_by_address = consolidation_subset_gdf.groupby('addr_normalized_key')[
            'normalized_text'
        ]
    ...
    def _apply_canonical_mapping(self, gdf_with_canonicals: cudf.DataFrame) -> cudf.DataFrame:
        ...
        result_gdf['normalized_text'] = result_gdf['canonical_name'].fillna(
            result_gdf['normalized_text']
        )
        ...
        logger.info(
            f'Applied canonical names: {num_names_changed:,} records had their names changed.'
        )
        return result_gdf

    def _log_consolidation_examples(
        self, grouped_names: cudf.Series, canonical_names: cudf.Series, max_examples: int = 5
    ):
        ...
        examples_df = cudf.DataFrame(
            {'all_names': grouped_names, 'canonical_name': canonical_names}
        )
    ...
    def _log_consolidation_summary(self) -> None:
        ...
        if stats.get('total_addresses', 0) > 0:
            ...
            logger.info(
                f'Addresses with multiple names: {stats["addresses_with_multiple_names"]:,}'
            )
            ...
            if stats.get('empty_addresses_skipped', 0) > 0:
                logger.info(
                    f'Records with empty addresses (skipped): {stats["empty_addresses_skipped"]:,}'
                )

            if stats.get('unique_before', 0) > 0 and stats.get('unique_after', 0) > 0:
                reduction = stats['unique_before'] - stats['unique_after']
                reduction_pct = (
                    (reduction / stats['unique_before']) * 100 if stats['unique_before'] > 0 else 0
                )
            ...
            if stats['addresses_with_multiple_names'] > 0:
                effectiveness = (
                    (stats['names_changed'] / stats['records_affected']) * 100
                    if stats['records_affected'] > 0
                    else 0
                )
                logger.info(
                    f'Consolidation effectiveness: {effectiveness:.1f}% of affected records were changed to the canonical name'
                )
        else:
