...
from .config import ColumnConfig, ValidationConfig, VectorizerConfig
from .utils import (
...
class AddressProcessor:
    ...
    def _combine_address_columns(self, gdf: cudf.DataFrame) -> cudf.Series:
        ...
        combined_address = (
            combined_address.str.replace(self.SEPS_PATTERN.pattern, ' ', regex=True)
    ...
    def _parse_address_series(self, address_series: cudf.Series) -> cudf.DataFrame:
        ...
        if failures > 0:
            ...
            logger.warning(f'Failed to parse {failures:,} addresses ({failure_rate:.1%})')
    ...
    def _add_parsed_columns_to_gdf(
        self, gdf: cudf.DataFrame, parsed_df: cudf.DataFrame
    ) -> cudf.DataFrame:
        """
    ...
    def _consolidate_similar_addresses(self, gdf: cudf.DataFrame) -> cudf.DataFrame:
        ...
        if consolidation_map is not None and not consolidation_map.empty:
            ...
            out_gdf['addr_normalized_key'] = out_gdf['addr_normalized_key'].replace(
                consolidation_map
            )
            ...
            reduction_pct = (
                (unique_before - unique_after) / unique_before if unique_before > 0 else 0
            )
    ...
    def _get_consolidation_map(
        ...
        pass_rate = (
            n_pairs_after_validation / n_pairs_before_validation
            if n_pairs_before_validation > 0
            else 0
        )
        logger.info(
        ...
        canonical_map_df = self._determine_canonical_representatives(
            candidates_gdf=unique_addresses_df, key_col=key_col
        )
        ...
        final_map_df = unique_addresses_df[[key_col, 'component_id']].merge(
            canonical_map_df, on='component_id', how='inner'
        )
    ...
    def _validate_address_pairs(
        ...
        pairs_with_components_df = matched_pairs_df.merge(
            unique_addresses_df[component_cols], left_on='source', right_index=True, how='left'
        ).rename(
            columns={
        ...
        pairs_with_components_df = pairs_with_components_df.merge(
            unique_addresses_df[component_cols], left_on='destination', right_index=True, how='left'
        ).rename(
            columns={
        ...
        valid_pairs_mask = cudf.Series(
            [True] * len(pairs_with_components_df), index=pairs_with_components_df.index
        )
        ...
        violates_street_num_rule = cudf.Series(
            [False] * len(pairs_with_components_df), index=pairs_with_components_df.index
        )
        ...
        if both_are_numeric.any():
            ...
            street_num_i = pairs_with_components_df['street_number_i'][both_are_numeric].astype(
                'int32'
            )
            street_num_j = pairs_with_components_df['street_number_j'][both_are_numeric].astype(
                'int32'
            )
        ...
        if enforce_state_boundaries:
            ...
            state_i_present = (
                pairs_with_components_df['state_i'] != ''
            ) & pairs_with_components_df['state_i'].notna()
            state_j_present = (
                pairs_with_components_df['state_j'] != ''
            ) & pairs_with_components_df['state_j'].notna()
            both_states_present = state_i_present & state_j_present

            # For pairs where both states are present, check if they are different.
            states_mismatch = (
                pairs_with_components_df['state_i'] != pairs_with_components_df['state_j']
            )
