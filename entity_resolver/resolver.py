...
class EntityResolver:
    ...
    def save_model(self, directory_path: str) -> None:
        ...
        if not self._is_fitted:
            raise RuntimeError(
                "Cannot save unfitted resolver. Call fit() first."
            )
    ...
    def _execute_training_pipeline(self, gdf: cudf.DataFrame) -> None:
        ...
        if self.config.columns.address_cols:
            ...
            self.logger.info("Step 2/7: Skipping address processing (no address columns configured)")
    ...
    def _execute_prediction_pipeline(self, gdf: cudf.DataFrame) -> cudf.DataFrame:
        ...
        if self.config.columns.address_cols:
            ...
            self.logger.info("Step 2/5: Skipping address processing (no address columns configured)")
    ...
    def get_review_dataframe(self) -> pd.DataFrame:
        ...
        if self.resolved_gdf_ is None:
            raise RuntimeError(
                "No resolved data available. "
                "Run fit_transform() or transform() first."
            )
    ...
    def _prepare_gpu_dataframe(self, df: pd.DataFrame) -> cudf.DataFrame:
        ...
        try:
            ...
            self.logger.debug(f"Successfully allocated {gpu_df.memory_usage().sum() / 1e6:.2f} MB on GPU")
            return gpu_df
