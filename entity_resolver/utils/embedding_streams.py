...
class TextStreamSet:
    ...
    tfidf: cudf.Series
    semantic: cudf.Series
    phonetic: cudf.Series | None = None
...
def prepare_text_streams(
    gdf: cudf.DataFrame, use_address_in_encoding: bool = True
) -> AllTextStreams:
    """
    ...
    **ADDRESS-ONLY Context** (address):
    ...
    logger.debug(
        f"Loaded entity names from 'normalized_text' column: {len(entity_name_text):,} records"
    )
    ...
    if 'addr_normalized_key' in gdf.columns:
        records_with_address = (
            gdf['addr_normalized_key'].notna() & (gdf['addr_normalized_key'] != '')
        ).sum()
        logger.debug(
    ...
    if use_address_in_encoding:
        ...
        semantic_address_part = (
            (street_name_series + ' ') + (city_series + ' ') + state_series
        ).str.strip()
    ...
    semantic_text_combined = (
        SEMANTIC_COMBINED_PREFIX + entity_name_text + ' ' + semantic_address_part
    ).str.strip()
    ...
    tfidf_text_combined = (weighted_name_combined + ' ' + tfidf_address_part).str.strip()
    ...
    tfidf_text_name_only = _repeat_series_with_space(
        entity_name_text, NAME_REPETITIONS_NAME_ONLY
    ).str.strip()
    ...
    semantic_text_name_only = (SEMANTIC_NAME_PREFIX + entity_name_text).str.strip()
    ...
    weighted_street_name = _repeat_series_with_space(
        street_name_series, STREET_NAME_REPETITIONS_ADDRESS
    )
    ...
    combined_stream_set = TextStreamSet(
        tfidf=tfidf_text_combined, semantic=semantic_text_combined, phonetic=phonetic_text_combined
    )
    ...
    all_streams_container = AllTextStreams(
        combined=combined_stream_set, name=name_only_stream_set, address=address_only_stream_set
    )
    ...
    def _validate_series_length(series: cudf.Series | None, context: str, stream: str):
