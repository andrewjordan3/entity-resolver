...
def ensure_finite_matrix(
    ...
    *,
    ...
    copy: bool = True,
) -> cupy.ndarray | cpx_sparse.spmatrix:
    """
    ...
    # --- Sparse Matrix Path ---
        ...
        if replace_non_finite:
            ...
            raise FloatingPointError(
                'Non-finite (NaN or Inf) values detected in the sparse matrix data.'
            )
...
def winsorize_matrix(
    ...
    *,
    ...
    # --- Sparse Matrix Path ---
        ...
        logger.debug(
            f'Winsorizing sparse matrix of shape {matrix.shape} (nnz={matrix.nnz}) with limits={limits}'
        )
...
def scale_by_frobenius_norm(
    ...
    *,
    ...
    copy: bool = True,
) -> tuple[cupy.ndarray | cpx_sparse.spmatrix, float]:
    """
...
def prune_sparse_matrix(
    ...
    *,
    ...
    col_energy = cupy.bincount(
        target_matrix.indices, weights=(target_matrix.data**2), minlength=n_features
    )
    ...
    logger.debug(
        ...
        int(kept_column_indices.size),
        int(n_features),
        int(min_df),
        int(max_df),
        float(energy_cutoff_ratio),
    )
