...
def normalize_rows(
    matrix: cupy.ndarray | cpx_sparse.spmatrix,
    *,
    epsilon: float | None = None,
    copy: bool = True,
    return_stats: bool = False,
) -> (
    cupy.ndarray | cpx_sparse.csr_matrix | tuple[cupy.ndarray | cpx_sparse.csr_matrix, bool, float]
):
    """
    ...
    if isinstance(matrix, cupy.ndarray):
        ...
        logger.debug(
            f'Dense path: {n_valid}/{matrix.shape[0]} rows will be normalized; '
        ...
        normalized_matrix = cupy.where(valid_rows_mask, normalized_matrix / row_norms, 0.0)
    ...
    if cpx_sparse.isspmatrix(matrix):
        logger.debug(
            f'Normalizing sparse matrix of shape {matrix.shape} with epsilon={epsilon:.1e}'
        )
        ...
        logger.debug(
            f'Sparse path: {n_valid}/{matrix.shape[0]} rows will be normalized; '
            f'{matrix.shape[0] - n_valid} will be zeroed.'
        )
    ...
    # --- Invalid Type ---
    raise TypeError(
        f'Input must be a cupy.ndarray or cupyx.scipy.sparse.spmatrix, but got {type(matrix)}'
    )
...
def center_kernel_matrix(
...
) -> cupy.ndarray | tuple[cupy.ndarray, dict[str, Any]]:
    """
    ...
    if not isinstance(kernel_matrix, cupy.ndarray):
        raise TypeError(f'kernel_matrix must be a CuPy ndarray, got {type(kernel_matrix).__name__}')
    if kernel_matrix.ndim != 2:
        raise ValueError(f'Kernel matrix must be 2-dimensional, got shape {kernel_matrix.shape}')
    n_rows, n_cols = kernel_matrix.shape
    if n_rows != n_cols:
        raise ValueError(f'Kernel matrix must be square. Got shape ({n_rows}, {n_cols})')
    if n_rows == 0:
    ...
    if logger.isEnabledFor(logging.DEBUG):
        ...
        verification_tolerance = (1e-5 if working_dtype == cupy.float32 else 1e-10) * max(
            1.0, float(cupy.abs(kernel_matrix).max())
        )
...
def center_kernel_vector(
    ...
    if not isinstance(kernel_vector, cupy.ndarray):
        raise TypeError(f'kernel_vector must be a CuPy ndarray, got {type(kernel_vector).__name__}')
    ...
    try:
        ...
        raise TypeError(f'training_kernel_grand_mean must be convertible to float: {e}') from e
    ...
    if input_was_1d:
        ...
        raise ValueError(f'kernel_vector must be 1D or 2D, got shape {kernel_vector.shape}')
    ...
    if logger.isEnabledFor(logging.DEBUG):
        ...
        expected_sum = (
            n_train_samples * training_kernel_grand_mean - training_kernel_row_means.sum()
        )
...
def get_top_k_positive_eigenpairs(
    ...
    try:
        ...
        raise ValueError(f'Failed to decompose matrix. It may be singular or ill-conditioned: {e}') from e
    ...
    if num_positive >= k:
        ...
        final_eigenvalues = cupy.pad(
            positive_eigenvalues, pad_width=(0, pad_width), mode='constant', constant_values=0
        ).astype(working_dtype)
...
def balance_feature_streams(
    ...
    logger.debug(
        f'Balancing streams with scaling factors: {", ".join(f"{stream}: {scale:.4f}" for stream, scale in logging_dict.items())}'
    )
...
def compute_average_gram_matrix(
    ...
    for i, emb in enumerate(normalized_embeddings):
        ...
        if max_index >= emb.shape[0]:
            raise ValueError(
                f'Index {max_index} is out of bounds for embedding {i} (size: {emb.shape[0]}).'
            )
    ...
    try:
        ...
        raise ValueError(
            'Failed to stack embeddings. Ensure all matrices have the same shape.'
        ) from e
    ...
    logger.info(
        f'Average Gram matrix computed with shape {average_gram_matrix.shape} and diagonal mean {diagonal_mean:.4f}.'
    )
...
def project_out_of_sample_batch(
    ...
    if validate_inputs:
        ...
        for i, (embedding, anchor_block) in enumerate(zip(normalized_embeddings, anchor_samples, strict=True)):
        ...
        if len(row_means) != n_anchors:
            raise ValueError(
                f"row_means length {len(row_means)} doesn't match number of anchors {n_anchors}"
            )
    ...
    avg_kernel_matrix = cupy.zeros((batch_size, n_anchors), dtype=cupy.float32)
    ...
    for run_idx, (embedding_matrix, anchor_block) in enumerate(
        zip(normalized_embeddings, anchor_samples, strict=True)
    ):
    ...
    centered_kernel_matrix = center_kernel_vector(avg_kernel_matrix, row_means, grand_mean)
...
def create_consensus_embedding(
    ...
    if n_components > n_anchor_samples:
        raise ValueError(
            f'n_components ({n_components}) cannot exceed n_anchor_samples ({n_anchor_samples})'
        )
    ...
    if use_sampling:
        ...
        logger.debug(f'Sampling {n_anchor_samples} anchor points from {n_points} total points...')
        anchor_indices = cupy.sort(cupy.random.choice(n_points, n_anchor_samples, replace=False))
    else:
    ...
    logger.debug(
        f'Gram matrix statistics: min={gram_min:.4f}, max={gram_max:.4f}, mean={gram_mean:.4f}'
    )
    ...
    eigenvectors, eigenvalues = get_top_k_positive_eigenpairs(
        centered_gram, k=n_components, eigenvalue_threshold=eigenvalue_threshold
    )
...
def create_initial_vector(
    ...
    *,
    ...
    logger.debug(
        f'Creating initial vector of size {vector_size} with dtype={dtype} and seed={seed}'
    )
