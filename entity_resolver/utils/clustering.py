...
def _calculate_cluster_centroids(
    ...
    for cluster_id in cluster_ids_to_process:
        ...
        if num_members > max_samples_per_cluster:
            ...
            cluster_centroids[cluster_id] = cp.mean(
                embedding_vectors[sampled_indices],
                axis=0
            )
        else:
            # Use all points for small clusters
            cluster_centroids[cluster_id] = cp.mean(
                embedding_vectors[member_indices],
                axis=0
            )
...
def _find_candidate_pairs(
    ...
    for batch_start_idx in range(0, num_clusters, computation_batch_size):
        ...
        for batch_row_idx, full_col_idx in zip(
            high_similarity_pairs[0].tolist(),
            high_similarity_pairs[1].tolist()
        ):
            ...
            if full_col_idx > full_row_idx:
                merge_candidate_pairs.append(
                    (cluster_ids[full_row_idx], cluster_ids[full_col_idx])
                )
...
def _perform_detailed_check_and_union(
    ...
    for pair_idx, (cluster_id_1, cluster_id_2) in enumerate(candidate_pairs):
        ...
        sampled_indices_1 = cp.random.choice(
            members_cluster_1,
            size=sample_size_1,
            replace=False
        )
        sampled_indices_2 = cp.random.choice(
            members_cluster_2,
            size=sample_size_2,
            replace=False
        )
...
def _relabel_from_union_find(
    ...
    final_cluster_mapping = {
        cluster_id: find_root_cluster(cluster_id)
        for cluster_id in union_find_structure.keys()
    }
...
def merge_snn_clusters(
    ...
    valid_cluster_ids = [
        int(cluster_id) 
        for cluster_id in unique_cluster_labels.tolist() 
        if cluster_id != -1
    ]
