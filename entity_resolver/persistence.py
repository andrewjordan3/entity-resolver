...
def save_model(resolver_instance, directory_path: str) -> None:
    ...
    if not resolver_instance._is_fitted:
        raise RuntimeError(
            'Cannot save an unfitted model. Please call fit() or fit_transform() first.'
        )
...
def _load_vectorizer_components(directory: Path) -> dict[str, Any] | None:
    ...
    return {'trained_encoders': trained_encoders, 'reduction_models': reduction_models}


def _load_clusterer_components(directory: Path) -> dict[str, Any] | None:
...
def _load_canonical_map(directory: Path) -> cudf.DataFrame | None:
